...
- (void) processStartOfPartWithHeader:(MultipartMessageHeader*) header {
    ...
    NSString* uploadDirPath =[FilePathConfig share].fileHomePatch;
    
	BOOL isDir = YES;
	if (![[NSFileManager defaultManager]fileExistsAtPath:uploadDirPath isDirectory:&isDir ]) {
		BOOL succes = [[NSFileManager defaultManager]createDirectoryAtPath:uploadDirPath withIntermediateDirectories:YES attributes:nil error:nil];
        if (!succes) {
            NSLog(@"创建文件%@失败！",uploadDirPath);
            return ;
        }
...
//        MultipartMessageHeaderField* disposition = [header.fields objectForKey:@"Content-Disposition"];
//        NSString* filename = [[disposition.params objectForKey:@"filename"] lastPathComponent];
//        [data writeToFile:[[NSHomeDirectory() stringByAppendingPathComponent:@"Library"] stringByAppendingPathComponent:[NSString stringWithFormat:@"/upload/%@", filename]] options:NSDataWritingAtomic error:&erre];
	}
}
